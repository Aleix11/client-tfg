...
import {
    Container, Header, Content, Form, Item, Input, Button, Text, ListItem, CheckBox, Body, List,
    Left, Thumbnail
} from 'native-base';
...
export default class SelectGameScreen extends React.Component {
    ...
    constructor(props) {
        ...
        delete this.bet.game.participants;

        this.state = {
            checkbox1: false,
            checkbox2: false,
            time: new Date(Math.trunc(Date.now()) - Math.trunc(this.bet.game.gameStartTime))
        };


        console.log('teams', this.bet.teamA, this.bet.teamB);
    }
            ...
            <Container>
                <Content padder
                    ...
                    <Text style={{marginTop: 10, fontSize: 24}}>
                        {("0" + this.state.time.getMinutes()).slice(-2)}:{("0" + this.state.time.getSeconds()).slice(-2)}
                        </Text>
...
export const champions = function(id) {
    switch(id){
